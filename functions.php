...
function get_site_word_count_comparison_filtered() {
    ...
    return '本站原创文章的总字数为 ' . number_format($total_words) . ' 字（已刨除代码），已经接近 ' . $closest_book . ' 的篇幅了！';
}
...
function enqueue_livephotoskit_script() {
	wp_enqueue_script('livephotoskit-js', $GLOBALS['assets_path'] . '/extra-js/livephotoskit.js', array(), null, true);
}
...
function render_custom_live_photos_block($attributes) {
    ...
    $width = '100%';
    $height = '410px'; // Example fixed height, adjust as necessary
