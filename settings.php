	...
	//do something...
					...
					<tr>
						...
						<td>
							<select name="argon_update_source">
								<?php 
									// 获取当前选项，如果未设置则默认 'stop'
									$argon_update_source = get_option('argon_update_source', 'stop'); // 关键修改：设置默认值 'stop'
								?>
								<option value="stop" <?php selected($argon_update_source, 'stop'); ?>><?php _e('暂停更新 (不推荐)', 'argon');?></option>
								<option value="github" <?php selected($argon_update_source, 'github'); ?>>Github</option>
								<option value="fastgit" <?php selected($argon_update_source, 'fastgit'); ?>>Fastgit</option>
								<option value="cfworker" <?php selected($argon_update_source, 'cfworker'); ?>>CF Worker</option>
								<option value="solstice23top" <?php selected($argon_update_source, 'solstice23top'); ?>>solstice23.top</option>
							</select>
							<p class="description"><?php _e('如更新主题速度较慢，可考虑更换更新源。', 'argon');?></p>
							<p class="description"><?php _e('若在主题文件中魔改了大量代码（或者使用被魔改的这个主题），一旦更新将<b>直接覆盖掉</b>魔改的代码。推荐暂停更新', 'argon');?></p>
						</td>
					</tr>
					<tr>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">'0~1之间，1为不透明，数字越小透明度越高。<br>这层遮罩是覆盖在原有壁纸（即argon_banner_background，包括日间、夜间）上的，如果不用填0即可o(≧口≦)o</p>
                        </td>
                    </tr>
					...
					<td>
						...
						<p>输出样例（返回在页脚区域）：<b>本站原创文章的总字数为 125,424 字（已刨除代码），已经接近 《月亮与六便士》（毛姆） 的篇幅了！</b></p>
					...
					<td>
						...
						<img 
								...
								/>
								<p>如需了解更多，可参见 <a href="https://loneapex.cn/archives/2699" target="_blank">这篇文章</a> <br>如需更改字体，建议直接参考上面的博文中的源代码更改。在php中设置会有许多bug的，已经放弃了……</p>
					</td>
					</tr>
...
function argon_update_themeoptions(){
	...
	if ($_POST['update_themeoptions'] == 'true'){
		...
		argon_update_option('argon_mimosa_header_mask_opacity');
